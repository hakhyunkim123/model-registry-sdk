...
def create_model_version(
    ...
    if response.status_code != 201:
        ...
        return ModelInfo(**response.json().get("model_version"))


def get_model_version(name: str, version: str, detail: bool = True) -> ModelInfo:
    url = f"{MODEL_VERSION_DOMAIN}"
...
def search_model_versions(filter_string: Optional[str] = None, detail: bool = False) -> List[ModelInfo]:
    ...
    if response.status_code != 200:
        ...
        return [ModelInfo(**model_version) for model_version in model_versions]
